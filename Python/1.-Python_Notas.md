...
> [Mixins source](https://www.pythontutorial.net/python-oop/python-mixin/)

Es una clase que contiene métodos usados por otras clases sin tener relación parental (herencia) con dicha clase, además no posee la compleja semántica de herencia múltiple.

Tipos comunes de Mixins.

* Funcionalidad adicional: Agrega métodos o atributos específicos a una clase.
* Comportamiento de protocolo: Define un comportamiento común para una serie de clases que implementan un protocolo o una interfaz.
* Utilidades de métodos estáticos: Proporciona métodos estáticos que se pueden utilizar en varias clases.
* Logging: Agrega funcionalidad de registro a una clase.
* Serialización: Define métodos para serializar y deserializar objetos.
* Validación de datos: Agrega métodos para validar los datos de una clase.


Utilizada para compartir funcionalidad entre diferentes clases, evitando repetir el mismo código una y otra vez, agrupando ese código en una clase y esta se herede a las que las requiera.

Recordar que los métodos heredados son buscados en las clases relacionadas de izquierda a derecha. Primero debe declararse las clases Mixins y luego la/s clase/s base/s.
...
Mixin2
>>> MyClass.__mro__
(<class '__main__.MyClass'>, <class '__main__.Mixin2'>, <class '__main__.Mixin1'>, <class 'object'>)
```
...
> [Simple personalización de clases - doc](https://docs.python.org/3/whatsnew/3.6.html#pep-487-simpler-customization-of-class-creation)

En Python 3.6, [PEP 487](https://www.python.org/dev/peps/pep-0487/) se agregó el método `__init_subclass__`, que simplifica y extiende personalizaciones de clases sin usar metaclases. Permitiendo crear plugins simples.
...
El patrón de diseño *Factory* permite un enfoque diferido. Fábricas pueden tomar múltiples formas, algunas no muy especiales.

Las clases son objetos <u>primera clase</u>, son fácilmente pasables alrededor de un programa, almacenarlos en una estructura, etc. Puedes pasar clases a funciones que generen cosas de objetos arbitrariamente, las funciones a veces son llamadas **fábricas** en círculos de diseño POO. Son trivial en Python, no tanto en lenguajes C++.

La función *fábrica* pasa la clase y los argumentos a la clase respectiva, creando la nueva instancia y retornándola. Usando los llamados especiales *"varargs"*.
...
Son una forma especifica de administrar o aumentar el código para funciones y clases, toman la forma de objetos llamables o  *callables* (como funciones) que procesan otros objetos llamables  (*callables*). Permiten insertar automáticamente código al final de la sentencia de función `def` y clase `class`.
...
* Call Proxies  :  decoradores en funciones interceptan los llamados y los procesan, usualmente pasando el *__call__* en la función original para ejecutar la acción.

* Interface Proxies  :  decoradores en clases interceptan "llamados de creación instancia" y los procesa cuando se requiere, usualmente pasando el *__call__* en la clase original para crear una instancia administrada.
...
La línea `def onCall(*args, **kwargs): `, llama la función original y sus argumentos que ella reciba. Debiendo retornar la función con dichos argumentos, `return func(*args, **kwargs)`
...
Un decorador toma un argumento para decorarlo. El problema radica cuando se pasan argumentos arbitrarios.

```python
def decoratorFactory(message):
    def decorator(func):
        def wrapper_func(*args, **kwars):
            print('The decorator watns to tell you: {}'.format(message))
...
Este ejemplo de decorador **debe recibir un argumento** de lo contrario dará error. 
...
Si este decorador de clase implementa `__call__()` es posible usarlo como si fuera una función.
...
Si se quiere decorar métodos se debe definir el método `__get__(self, instance, cls)`. <u>Al decorar un método, este se comparte con todas las instancias creadas de dicha clase al que se le aplique</u>.

* Decorar un método de la clase
...
Son libres de mejorar una clase o `return` un objeto arbitrario para sustituirla, un protocolo con posibilidades de personalización basadas en clases casi ilimitadas.
...
2. Estableciendo dentro de la clase.
...
Se pueden crear decoradores de función o clase y aplicarlos a la clase deseada.

* Decorador de función anidada para una clase.
...
* Decorador de clase anidada para una clase.
...
El decorador de clase es libre para retornar un objeto "proxy" (wrapper) que intercepte los llamados *`__call__()`* de construcción de instancias.
...
De manera similar, se puede crear un decorador de clase y aplicarlo en una función o clase.
...
Es un patrón de diseño que permite asegurar que se utilice una sola instancia de una clase, restringe la creación de instancias de una clase. Debido a que los decoradores pueden interceptar todas los llamados de creación o mejorar la interfaz de esas instancias.
...
Permite la restricción de inicializaciones de clases a una por objeto/instancia, aplicada dentro de una clase creada.
...
Otra forma es decorar la clase completa.

```python
class Singleton:
    """
    Usada como decorador, no como metaclase.
    Las clases decoradas, solo usa `__init__(self)` como constructor.
    Para obtener la instancia Singleton, se debe usar método `.Instance`
    
    La clase decorada no puede heredar.
    """
    ...
    def Instance(self):
        """
        Retorna instancia singleton. Desde su primer llamado, este crea una nueva
        instancia de la clase decorada y llama al método `__init__`.
        Todas las secundaias retorna la instancia ya creada.
        """
        try:
    ...
    def __call__(self):
        raise TypeError('Singletons must be acceded through `Instance()`.')
    
    def __instancecheck__(self, instance):
        return isinstance(instance, self._decorated)
```

Usando la clase `Singleton` decorando una clase.
...
x.getName()  # I am Single 
y.getName()  # I am Single
```
...
> Metaclases proporcionan controlar cómo las clases son inicializadas o creadas.
...
En Python 2.X, a la clase que se aplique la metaclase se debe agregar el atributo `__metaclass__ = MetaClass`.
...
...     def __new__(cls, class_name, class_parent, class_dict):
...             print('New Class: ', class_name)
...             new_class = super().__new__(cls, class_name, class_parent, class_dict)
...             return new_class
... 
...
Se puede implementar singleton utilizando metaclases.
...
En Python 3.1 y posteriores, soportan múltiples `with` (anidados) separados por coma (*,*).
...
| Flag | Descripción |
|-|-|
| re.IGNORECASE, re.I | ignora case sensitive. |
| re.DOTALL, re.S | incluye todo hasta el punto `.` incluso una nueva línea. |
| re.MULTILINE, re.M | toma el comienzo de la línea `^` y el final de la línea `$`.
| re.DEBUG | información de depuración. |
